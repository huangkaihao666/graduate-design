...
```bash
# 前置：确保 Ollama 已安装并拉取模型（只需执行一次）
ollama pull nomic-embed-text

# 启动 opinion-service
cd opinion-service
python3 -m venv .venv && source .venv/bin/activate  # 首次建虚拟环境
pip install -r requirements.txt                      # 首次安装依赖
uvicorn main:app --reload --port 8001               # 启动，端口 8001
```

服务启动后访问 `http://localhost:8001/docs` 可查看接口文档。RAG 服务不可用时 NestJS 自动降级，不影响主功能。
...
| Phase 2-A：AI 共情师                | 🔶 部分完成 | 核心对话 + RAG 记忆层 + 冷启动开场白已实现；新建会话触发归档；5分钟定时归档          |
...
| Phase 5：本地 RAG 服务              | ✅ 已完成   | opinion-service 完整实现；第三层（弹幕过滤）接入辩论室；第四层（情绪记忆）接入共情师 |
