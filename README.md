...
| 软件           | 最低版本 | 推荐版本             | 说明                                   |
| -------------- | -------- | -------------------- | -------------------------------------- |
| **Node.js**    | 18.0     | 20.x LTS 或 22.x LTS | JavaScript 运行时环境                  |
| **pnpm**       | 8.0      | 10.29.3 或更新       | Monorepo 包管理工具（必需）            |
| **npm**        | 10.0     | 10.0 或更新          | 用于安装 pnpm（`npm install -g pnpm`） |
| **NestJS CLI** | 11.0     | 11.0 或更新          | NestJS 命令行工具（可选）              |
| **MySQL**      | 8.0      | 8.0 或 5.7           | 后端数据库（需单独安装或使用 Docker）  |
| **Docker**     | -        | 最新版本（推荐）     | 用于运行 MySQL 容器（可选）            |

> **重要提示**：
>
> - 本项目使用 **pnpm** 作为包管理器，**不支持 npm 或 yarn**
> - 后端使用 **NestJS 11** 框架，需要 Node.js 18+
> - 后端使用 **Prisma ORM** 和 **MySQL 8.0** 数据库
> - 建议使用 Docker 运行 MySQL，避免版本和配置问题
...
后端使用 Prisma ORM 和 MySQL 数据库，需要先启动数据库和初始化数据库迁移：
...
> 详细说明见 [后端数据库配置指南](./backend/README.md#快速开始)
...
这会同时启动前端（`http://localhost:5173`）和后端（`http://localhost:3000/api/v1`）
